...
a square on the diagonal of each self quadrant, and nothing in the cross quadrants. The line dividing the two blocks is white
...
Only those two. The cross quadrants are left clean: whatever is drawn there — a rectangle
per (reference, read) interval pair, or each read interval at the reference span the
